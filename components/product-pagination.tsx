...
export default function ProductPagination({ currentPage, totalPages, categoryParam, stockParam}: Props) {
   const stock = stockParam ?? '';
   console.log(`stock url : ${stock}`);
  return (
      ...
      <div className="flex items-center gap-2">
        ...
        {currentPage > 1 && (
          <Link
            ...
            href={`/?page=${currentPage - 1}&category=${categoryParam}${stock}`}
          >
            <ChevronLeft size={18} />
          </Link>
        )}
        ...
        {currentPage + 2 <= totalPages && (
          <Link
            href={`/?page=${currentPage + 2}&category=${categoryParam}${stock}`}
            className="flex h-10 w-10 items-center justify-center rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-100"
          >
            {currentPage + 2}
          </Link>
        ...
        {currentPage + 1 <= totalPages && (
          <Link
            href={`/?page=${totalPages}&category=${categoryParam}${stock}`}
            className="flex h-10 w-10 items-center justify-center rounded-md border border-gray-300 bg-white text-gray-700 hover:bg-gray-100"
          >
            {totalPages}
          </Link>
