...
export default class ProductService {
  ...
  static async getProducts(currentPage: number, categoryParams: string, stockParams: string): Promise<ApiResponse<ProductsResponse>> {
    try {
      ...
      const stock = stockParams ?? '';

            
      const response = await fetch(`${API_URL}/products?_page=${currentPage}&_limit=${defaultLimit}&_sort=id&_order=desc&_expand=category${category}${stock}`, {
        method: 'GET',
      });
